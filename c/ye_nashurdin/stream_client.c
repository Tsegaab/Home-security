...
void quit_s(char* msg, int retval);
...
int main(int argc, char** argv) {
	...
	if (argc != 5) {
		quit_s("Usage: stream_client <server_ip> <server_port> <width> <height>",
				0);
	}
	...
	if (pthread_create(&thread_c, NULL, streamClient, NULL )) {
		quit_s("pthread_create failed.", 1);
	}
	...
	if (pthread_cancel(thread_c)) {
		quit_s("pthread_cancel failed.", 1);
	}
